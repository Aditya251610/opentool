        ...
        {[
          {
            ...
            value: stats.loading ? '—' : String(stats.connectedCount),
            sub: `of ${Object.keys(PROVIDERS).length} providers`,
            accent: true,
          },
          {
            label: 'Available Actions',
            value: stats.loading ? '—' : String(stats.totalTools),
            sub: 'across all providers',
          },
          {
            label: 'API Keys',
            value: stats.loading ? '—' : String(stats.keyCount),
            sub: 'active keys',
          },
          {
            label: 'Server Status',
            value: stats.loading ? '—' : stats.serverOnline ? 'Online' : 'Offline',
            sub: stats.serverOnline ? 'All systems operational' : 'Cannot reach server',
          },
        ].map((stat) => (
          ...
          <Card highlighted>
            ...
            <div className="space-y-2.5">
              <SetupStep
                ...
                desc="Head to Tools and link a provider like GitHub or Slack"
                icon={<Wrench size={14} />}
                done={stats.connectedCount > 0}
              />
              <SetupStep
                step={2}
                title="Generate an API key"
                desc="Create a key in API Keys to authenticate your agents"
                icon={<Key size={14} />}
              ...
              <SetupStep
                ...
                desc="Add OpenTool as an MCP server in Claude Desktop or Cursor"
                icon={<Zap size={14} />}
              />
            </div>
          </Card>
                ...
                {stats.loading ? (
                  ...
                  <>
                    <span className="relative flex h-2 w-2">
                      <span
                        className={`animate-ping absolute inline-flex h-full w-full rounded-full ${stats.serverOnline ? 'bg-[#22c55e]' : 'bg-[#ef4444]'} opacity-75`}
                      />
                      <span
                        className={`relative inline-flex rounded-full h-2 w-2 ${stats.serverOnline ? 'bg-[#22c55e]' : 'bg-[#ef4444]'}`}
                      />
                    </span>
                    <span
                      className={`text-[11px] font-medium ${stats.serverOnline ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}
                    >
